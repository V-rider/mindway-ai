...
import { Upload, Check, AlertCircle, Loader2, Users, Plus, Trash2, SelectAll, X } from "lucide-react";
...
export const UploadForm: React.FC = () => {
  ...
  const handleFiles = (newFiles: File[]) => {
    // Filter valid files (images and PDFs only)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${files.length} tests uploaded for ${selectedStudents.length} students`,
        variant: "default"
      });
    ...
    <div className="max-w-4xl mx-auto">
      <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {files.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
                  Drag and drop multiple test files here, or click to browse.
                  Upload all tests at once for efficient processing.
                </p>
                
                <label 
                  htmlFor="file-upload" 
                  className="cursor-pointer px-6 py-3 bg-purple-600 hover:bg-purple-700 focus:ring-4 focus:ring-purple-300 text-white font-medium rounded-lg text-sm transition-colors"
                >
                  Choose Files
                  <input
                    id="file-upload"
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="inline-flex items-center px-3 py-1.5 bg-green-100 hover:bg-green-200 dark:bg-green-900/30 dark:hover:bg-green-900/50 text-green-700 dark:text-green-300 text-sm font-medium rounded-lg transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center px-3 py-1.5 bg-red-100 hover:bg-red-200 dark:bg-red-900/30 dark:hover:bg-red-900/50 text-red-700 dark:text-red-300 text-sm font-medium rounded-lg transition-colors"
                  >
            ...
            {files.length > 0 && selectedStudents.length > 0 && (
              <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                <p className="text-sm text-blue-800 dark:text-blue-300">
                  <strong>Batch Upload:</strong> {files.length} files will be processed for {selectedStudents.length} selected students. 
                  The system will automatically distribute and analyze the tests.
                </p>
              </div>
            )}
          </div>
        ...
        <motion.button
          ...
          className="w-full py-4 px-6 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 focus:ring-4 focus:ring-purple-300 text-white font-medium rounded-lg text-lg transition-all disabled:opacity-70 disabled:cursor-not-allowed"
        ...
        >
          {uploading ? (
            ...
            `Upload & Analyze ${files.length} Tests for ${selectedStudents.length} Students`
          )}
        </motion.button>
      </form>
    </div>
